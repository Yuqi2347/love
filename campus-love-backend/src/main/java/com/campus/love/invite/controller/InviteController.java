...
public class InviteController {
    ...
    public Result<IPage<InviteResponse>> getInviteList(
            ...
            @Parameter(description = "仅公开邀约（发现页用）") @RequestParam(required = false) Boolean publicOnly,
            @Parameter(description = "排序：recommend=推荐，默认按时间") @RequestParam(required = false) String sort,
            @Parameter(description = "页码，从 1 开始") @RequestParam(defaultValue = "1") Integer page,
            @Parameter(description = "每页数量") @RequestParam(defaultValue = "10") Integer size) {
        IPage<InviteResponse> invites = inviteService.getInviteList(type, status, timeRange, keyword, publicOnly, page, size, sort);
